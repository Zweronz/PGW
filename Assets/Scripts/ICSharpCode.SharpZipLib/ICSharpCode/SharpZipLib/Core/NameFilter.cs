		...
		private void Compile()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				if (array[i] != null && array[i].Length > 0)
				{
					...
					if (flag)
					{
						inclusions_.Add(new Regex(pattern, RegexOptions.IgnoreCase | RegexOptions.Singleline));
					}
					else
					{
						exclusions_.Add(new Regex(pattern, RegexOptions.IgnoreCase | RegexOptions.Singleline));
					}
				}
			}
		}
